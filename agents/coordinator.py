"""
agents/coordinator.py — Top-level agent coordinator.

PURPOSE:
    The AgentCoordinator is the CENTRAL ORCHESTRATOR of the TRUCE-PT
    multi-agent system. It replaces the older linear orchestrator.py pipeline
    with an agent-based architecture where each stage of the pentest is
    handled by a specialized agent.

    The coordinator:
      1. Instantiates all 9 agents, giving each one a reference to the
         shared SessionContext.
      2. Executes the 9-step pipeline in sequence, calling each agent's
         execute() method at the appropriate time.
      3. Collects results from each agent and decides whether to proceed
         (e.g., aborts if discovery finds zero devices).
      4. Handles conditional steps (Safety Officer only runs when OT
         devices are detected).
      5. Runs a post-execution Fleet Reasoner pass for hypothesis propagation.
      6. Finalizes and exports session results, PCF evidence, and the
         engagement ledger.

THE 9-STEP PIPELINE:
    Step 1: Discovery Agent      — Phases 0-1: passive recon + active host discovery.
    Step 2: Target Profiling     — Phases 2-3: fingerprinting + TIB tier assignment.
    Step 3: Fleet Reasoner       — Clustering: group similar devices into families.
    Step 4: Planner Agent        — PTG generation: build per-device attack plans.
    Step 5: Safety Officer       — OT safety review (CONDITIONAL: only if OT detected).
    Step 6: Tool Orchestrator    — PTG execution: run scans/probes/exploits.
    Step 7: Impact Monitor       — Post-execution health check across all devices.
    Step 8: Validator Agent      — Finding validation using oracle recipes.
    Step 9: Evidence Agent       — Proof bundle construction + PCF checkpoint.
    [POST]: Fleet Reasoner       — Hypothesis propagation (if clusters exist).

MESSAGE BUS ARCHITECTURE:
    Agents communicate via a shared MessageBus (in-process, per-role queues).
    The pipeline is sequential, but agents can send messages to agents that
    will execute later in the pipeline. For example:
      - TargetProfiling sends an ALERT to SafetyOfficer at Step 2, and the
        SafetyOfficer processes it when it runs at Step 5.
      - ToolOrchestrator sends RESULTs to Evidence at Step 6, and the
        Evidence Agent processes them at Step 9.
    The message bus decouples agents so they don't need direct references
    to each other — they only know about AgentRoles.
...
# Import agent roles for type references and message bus addressing
from agents.base import AgentRole
from agents.session_context import SessionContext

# Import all 9 agent classes — each handles one pipeline step
...
class AgentCoordinator:
    ...
    def __init__(self, context: SessionContext):
        # Store the shared session context
        ...
        self.post_exploitation = PostExploitationAgent(context)  # Step 6.5
    ...
    def run(self, cmdp_policy=None, progress_cb=None) -> dict:
        ...
        # ── Step 6.5: Post-Exploitation (Reverse Shell + Lateral Movement) ──
        # Only runs if there are devices with exploitation findings
        exec_findings = execution_result.data.get("findings", 0)
        if exec_findings > 0:
            self._progress("[Step 6.5] Post-Exploitation — reverse shell, lateral movement, C2 scoring...")
            post_exploit_result = self.post_exploitation.execute()
            self._results["post_exploitation"] = post_exploit_result.data
            pe = post_exploit_result.data
            self._progress(
                f"           Shells: {pe.get('shells_established', 0)}/{pe.get('shells_attempted', 0)} | "
                f"Pivots: {pe.get('total_pivot_paths', 0)} ({pe.get('confirmed_pivots', 0)} confirmed) | "
                f"Kill chains: {pe.get('kill_chains', 0)}"
            )
            # Show attack graph summary
            ag = pe.get("attack_graph", {})
            if ag:
                self._progress(
                    f"           Attack graph: {ag.get('total_nodes', 0)} nodes, "
                    f"{ag.get('total_edges', 0)} edges, "
                    f"{ag.get('compromised_devices', 0)} compromised, "
                    f"{ag.get('high_value_targets', 0)} high-value targets"
                )
            # Show C2 risk distribution
            risk_dist = pe.get("risk_distribution", {})
            if risk_dist:
                risk_str = " | ".join(f"{k}:{v}" for k, v in risk_dist.items())
                self._progress(f"           C2 Risk: {risk_str}")
            # Show subnet risks
            for sr in pe.get("subnet_risks", [])[:3]:
                self._progress(
                    f"             {sr['subnet']}: {sr['risk_level']} risk "
                    f"({sr['confirmed_pivots']} confirmed pivots)"
                )
        else:
            self._progress("[Step 6.5] Post-Exploitation — skipped (no exploitation findings)")
            self._results["post_exploitation"] = {"skipped": True}
    ...
    def _finalize(self) -> dict:
        ...
        Assembles all session data into a single dict suitable for JSON
        export. Includes:
          - Session timing (start, end, duration).
          - Device count and tier summary.
          - PCF DAG integrity status.
          - Session metrics (findings, validations, instabilities, vetoes).
          - Per-agent results from each pipeline step.
          - Per-device summaries (IP, vendor, tier, ports, budget usage).
          - Safety compliance report (if OT mode was active).
          - Proof bundle count.
          - PTG summaries (node counts, completion status per device).

        Returns:
            Comprehensive session results dict.
        """
        ...
        # Include post-exploitation results (lateral movement, attack graph,
        # reverse shell risk scores) if the post-exploitation phase ran.
        pe_data = self._results.get("post_exploitation", {})
        if pe_data and not pe_data.get("skipped"):
            results["post_exploitation"] = {
                "shells_established": pe_data.get("shells_established", 0),
                "total_pivot_paths": pe_data.get("total_pivot_paths", 0),
                "confirmed_pivots": pe_data.get("confirmed_pivots", 0),
                "kill_chains": pe_data.get("kill_chains", 0),
                "attack_graph": pe_data.get("attack_graph", {}),
                "risk_distribution": pe_data.get("risk_distribution", {}),
                "subnet_risks": pe_data.get("subnet_risks", []),
            }

            # Add lateral movement and risk data per device
            lateral_results = self.post_exploitation.get_lateral_results()
            risk_results = self.post_exploitation.get_risk_results()

            for device in results.get("devices", []):
                ip = device.get("ip", "")
                # Add lateral movement data
                for lr in lateral_results:
                    if lr.device_ip == ip:
                        device["lateral_movement"] = lr.to_dict()
                        break
                # Add C2 risk score
                for rr in risk_results:
                    if rr.device_ip == ip:
                        device["c2_risk"] = rr.to_dict()
                        break

            # Add attack graph kill chains to results
            attack_graph = self.post_exploitation.get_attack_graph()
            if attack_graph:
                results["kill_chains"] = [
                    kc.to_dict() for kc in attack_graph.kill_chains[:10]
                ]
    ...
    def save_results(self, output_path: str = "results.json",
        ...
        Produces three files:
          1. results.json          — High-level session results and metrics.
          2. pcf_evidence.json     — The complete PCF DAG (evidence chain).
          3. engagement_ledger.json — Proof bundles + integrity verification.

        Args:
            output_path — Path for the results JSON file.
            pcf_path    — Path for the PCF DAG JSON file.
            ledger_path — Path for the engagement ledger JSON file.
        """
    ...
    def print_summary(self) -> None:
        ...
        Displays key metrics (duration, device count, findings, validations)
        and a per-tier breakdown of devices with their ports and budget usage.
        Uses ANSI bold codes for visual emphasis.
        """
        metrics = self.context.get_session_metrics()
...
def run_agent_scan(
    ...
    This is the top-level function that sets up the entire TRUCE-PT system
    and runs a complete engagement. It is called by the CLI (main.py) or
    can be invoked programmatically.

    SETUP STEPS:
      1. Load the OUI database (MAC-to-vendor lookup).
      2. Register all built-in IC-ToolSpec tools.
      3. Create the SessionContext (shared state).
      4. Create the AgentCoordinator (instantiates all agents).
      5. Optionally load a CMDP policy for intelligent action selection.
      6. Run the 9-step pipeline via coordinator.run().
      7. Save results, PCF evidence, and engagement ledger to disk.
      8. Print a human-readable summary to the console.

    Args:
        networks     — List of CIDR ranges to scan (e.g., ["192.168.1.0/24"]).
        oui_db_path  — Path to the OUI database file (optional).
        passive_only — If True, only perform passive recon (no active probing).
        max_threads  — Maximum thread count for concurrent scan phases.
        output_path  — Where to save the results JSON.
        pcf_path     — Where to save the PCF DAG JSON.
        use_cmdp     — Whether to use CMDP policy for action selection.
        exploit_all  — Force exploitation on ALL devices (incl. CRITICAL).

    Returns:
        Session results dict from the coordinator.
    """
    ...
    # names (e.g., "00:1A:2B" -> "Siemens"). This is used during discovery
